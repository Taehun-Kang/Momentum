...
-- Supabase 데이터베이스 스키마 (VECTOR 타입 제거 버전)
...
CREATE INDEX IF NOT EXISTS idx_api_usage_daily ON api_usage_logs(DATE(created_at), api_name);
...
BEGIN
  ...
  RAISE NOTICE 'Wave Team - Momentum 프로젝트 (VECTOR 타입은 향후 추가 예정)';
END $$; 